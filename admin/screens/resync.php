...
if ( isset( $_POST['mrs_gitdeploy_resync_all_files_submit'] ) ) {
    ...
    if ( ! isset( $_POST['mrs_gitdeploy_resync_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['mrs_gitdeploy_resync_nonce'] ) ), 'mrs_gitdeploy_resync_action' ) ) {
        wp_die( __( 'Nonce verification failed.', 'mrs-gitdeploy' ) );
    }

    // Check if the current user has permission to perform this action
    if ( ! current_user_can( 'manage_options' ) ) {
        wp_die( __( 'You do not have sufficient permissions to perform this action.', 'mrs-gitdeploy' ) );
    }

    if ( 'yes' === get_option( 'mrs_gitdeploy_resync_in_progress' ) ) {
        echo '<div class="error"><p>' . __('A Resync is already in progress. Check the Deployments tab!', 'mrs-gitdeploy') . '</p></div>';
    } else {
        // Calling the PHP function to resync
        mrs_gitdeploy_resync_all_files_from_github_repo();

        echo '<div class="updated"><p>' . __('ReSync Started!', 'mrs-gitdeploy') . '</p></div>';
    }
}
    ...
    <div style="display: flex; flex-direction: column; gap: 40px;">
        ...
        <div style="display: flex; gap: 40px; justify-content: space-between;">
            ...
            <div style="flex: 1; background-color: #f9f9f9; padding: 20px; border-radius: 8px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
                <h3 style="font-size: 20px; margin-bottom: 10px;"><?php esc_html_e( 'ReSync GitHub repo from WordPress codebase', 'mrs-gitdeploy' ); ?></h3>
                <p style="font-size: 14px; color: #555;">
                    <?php esc_html_e( 'With this option, the plugin will automatically create a GitHub commit using the WordPress codebase. This includes plugins, themes, mu-plugins, and the index.php file. It\'s a seamless way to keep your GitHub repository in sync with your WordPress environment.', 'mrs-gitdeploy' ); ?>
                </p>
                <div style="display: flex; align-items: center; gap: 15px; margin-top: 15px;">
                    <button id="resync-button" class="button button-primary"><?php esc_html_e( 'Update GitHub Repo from WP', 'mrs-gitdeploy' ); ?></button>
            ...
            <div style="flex: 1; background-color: #f9f9f9; padding: 20px; border-radius: 8px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
                <h3 style="font-size: 20px; margin-bottom: 10px;"><?php esc_html_e( 'ReSync WordPress codebase from GitHub repo', 'mrs-gitdeploy' ); ?></h3>
                <p style="font-size: 14px; color: #555;">
                    <?php esc_html_e( 'With this option, the plugin will automatically update your WordPress codebase with the code from your repo. This includes plugins, themes, mu-plugins, and the index.php file. It\'s a seamless way to keep your WordPress codebase in sync with your GitHub repository.', 'mrs-gitdeploy' ); ?>
                </p>

                <div style="display: flex; align-items: center; gap: 15px; margin-top: 15px;">
                    <button type="button" id="resync-all-files-button" class="button button-secondary">
                        <?php esc_html_e( 'Update WP from GitHub Repo', 'mrs-gitdeploy' ); ?>
                    </button>
                </div>
            </div>
            ...
            <div id="confirmation-modal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background-color:rgba(0, 0, 0, 0.5); justify-content:center; align-items:center;">
                <div style="background-color:#fff; padding:20px; border-radius:8px; max-width:400px; text-align:center;">
                    <h3><?php esc_html_e( 'Are you sure?', 'mrs-gitdeploy' ); ?></h3>
                    <p>
                        <?php esc_html_e( 'This will delete all your existing plugins, themes, and mu-plugins files, and replace them with the ones from your GitHub repo.', 'mrs-gitdeploy' ); ?>
                        <br>
                        <span style="color: red;">
                            <b>
                                <?php esc_html_e( 'This action is not reversible.', 'mrs-gitdeploy' ); ?>
                            </b>
                        </span>
                    </p>
                    <div style="margin-top:20px;">
                        <form method="post">
                            <?php wp_nonce_field( 'mrs_gitdeploy_resync_action', 'mrs_gitdeploy_resync_nonce' ); ?>
                                <button name="mrs_gitdeploy_resync_all_files_submit" id="proceed-button" class="button button-primary"><?php esc_html_e( 'Yes, proceed', 'mrs-gitdeploy' ); ?></button>
                        </form>
                        </br>
                        <button id="cancel-button" class="button button-secondary"><?php esc_html_e( 'Cancel', 'mrs-gitdeploy' ); ?></button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div style="background-color: #f9f9f9; padding: 20px; border-radius: 8px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); text-align: center;">
            <h3 style="font-size: 20px; margin-bottom: 10px;"><?php esc_html_e( 'Manually Resync GitHub Repo', 'mrs-gitdeploy' ); ?></h3>
            <p style="font-size: 14px; color: #555;">
                <?php esc_html_e( 'Clicking the button below will download the complete WordPress codebase (plugins, themes, mu-plugins, and index.php). This can be manually uploaded to GitHub, which is helpful if there are issues with the GitHub API, such as rate limits.', 'mrs-gitdeploy' ); ?>
            </p>
            <div style="align-items: center; gap: 15px; margin-top: 15px;">
                <button id="generate-zip-btn" class="button button-secondary" style="margin-top: 15px;"><?php esc_html_e( 'Download WordPress Code', 'mrs-gitdeploy' ); ?></button>
